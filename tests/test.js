...
const AUTO_UPDATE_DEV_URL = 'https://api.dev.update.rocks/update/github.com/rllola/electron-example/stable/'
...
test('test debian release (DEV)', async t => {
    const response = await axios.get(AUTO_UPDATE_DEV_URL + 'linux/' + FAKE_APP_VERSION)
    console.log(response.data)
    t.is(response.data.name, `v${APP_VERSION}`)
    t.not(response.data.url, '', 'No release found')
});

test('test macOS release (DEV)', async t => {
    const response = await axios.get(AUTO_UPDATE_DEV_URL + 'darwin/' + FAKE_APP_VERSION)
    console.log(response.data)
    t.is(response.data.name, `v${APP_VERSION}`)
    t.not(response.data.url, '', 'No release found')
});

test('test window release (DEV)', async t => {
    const response = await axios.get(AUTO_UPDATE_DEV_URL + 'win32/' + FAKE_APP_VERSION)
    console.log(response.data)
    t.is(response.data.name, `v${APP_VERSION}`)
    t.not(response.data.url, '', 'No release found')
});